...
| `0` | `kube-prometheus-stack` | Prometheus/Grafana/Alertmanager (Phase 5) — the first real use of the **multi-source** Application pattern (public Helm chart + this repo's values file as a second source; see that file's own comments for why `ingress-nginx`/LocalStack below *don't* use this pattern yet). Its CRDs are **not** managed by this Application (`crds.enabled: false`) — Argo CD's own sync pipeline fails on them even with Server-Side Apply; they're installed once, manually, outside GitOps entirely. See `../infra/PHASE5-RUNBOOK.md` step 2 and `../docs/errors.md` for why. |
