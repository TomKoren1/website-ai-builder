...
- [x] **Fold `ingress-nginx`/LocalStack into Argo CD** (Phase 4) — `infra/up.ps1` still does the initial imperative bootstrap (Argo CD itself doesn't exist until Phase 4, so Phase 1 can't depend on it existing), but ongoing reconciliation is now Argo CD's via two new Applications adopting the already-running releases. Verified as a genuine zero-diff adoption (pinned to the exact live chart versions, confirmed every resource `Synced` with zero sync operations run) before enabling automated sync — no restart, no drift-correction action, on either component. See `argocd/README.md` and `docs/errors.md`.
...
- [ ] **`act_runner`/Gitea Actions aren't behind Argo CD** (Phase 4, Flow B) — same category ingress-nginx/LocalStack were in before the fix above; installed imperatively, not yet adopted.
