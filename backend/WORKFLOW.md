...
POST /projects/{id}/push  { files, commit_message }
  ...
  → INSERT into `deployments` (git_commit_sha, status="success", deployed_at)
```

**Where it's stored**: the actual site files live in Gitea from this point on (that's the real deployment artifact). `deployments` in Postgres is just a log of push attempts — Phase 4's CI/CD will later update `status` based on real build/deploy results instead of assuming success synchronously the way it does now.
...
| Generated site files (pushed) | The project's Gitea repo (`projects/project-<hex>`), branch `main` |
| Deploy history | `deployments` (metadata only — the files themselves are in Gitea, not duplicated into Postgres) |
